...
The agent serve sidecar (when enabled) listens on port `4096`. This is the `opencode serve` process managed by the daemon. See [OpenTelemetry](/docs/opentelemetry#agent-serve-lifecycle) for details on the serve lifecycle.
...
server, see [MCP Server](/docs/mcp-server) for details.
...
`loop-task http-host`. See [CLI Reference](/docs/cli-reference#loop-task-http-host-address)
for details.
