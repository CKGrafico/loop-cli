...
When the verify task fails, its stderr (compiler errors, test failures, lint violations) is captured into `{{output}}`. The fix task uses `{{output}}` to see exactly what went wrong. No custom JSON wrappers needed.
