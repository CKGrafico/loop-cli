...
docker run -it --rm \
  ...
  node:20-slim npx @plainconceptsplatform/loop-task
```
...
docker run -d --name loop-daemon \
  -v loop-task-data:/root/.loop-cli \
  node:20-slim npx @plainconceptsplatform/loop-task start

docker exec loop-daemon npx @plainconceptsplatform/loop-task new 30m -- npm test
```
...
```dockerfile
FROM node:20-slim
RUN npm install -g @plainconceptsplatform/loop-task
