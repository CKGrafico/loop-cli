...
## Agent serve not starting

When the daemon starts, it attempts to start an `opencode serve` sidecar on port 4096. If this fails, agent tasks fall back to cold-start `opencode run` (slower but functional). Diagnose:
...
2. **Check if port 4096 is in use:**
   ```bash
   curl http://localhost:4096/global/health
   # If a different serve is already running, that's fine — the daemon reuses it
   ```

3. **Check daemon logs for serve errors:**
   ```bash
   # Look for "opencode-serve" or "agent-serve-manager" entries
   grep -i "serve" ~/.loop-cli/daemon.log
   ```

4. **If serve crashes repeatedly:** The daemon will restart it automatically. If it keeps crashing, check opencode's own logs for errors. You can also stop the daemon, manually run `opencode serve --port 4096`, then restart the daemon — it will detect and reuse the running serve.
